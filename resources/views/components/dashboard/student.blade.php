...
<div class="space-y-6 lg:col-span-2">
    <x-welcome.welcome-window section-name="Koncerty Vašich kapiel">
        @forelse($user->student->bands as $band)
            <p class="text-sm text-slate-500">Pozícia v kapele {{ Str::lcfirst($band->name) }}: {{ $user->student->specialization->name }}</p>
            @forelse($band->events as $event)
                <x-welcome.welcome-components.event-info :$event :$band/>
            @empty
                <p class="text-sm text-slate-500">
                    Kapela nie je prihlásená na žiadne udalosti
                </p>
            @endforelse
        @empty
            <p class="text-sm text-slate-500">
                Žiak nie je v žiadnej kapele
            </p>
        @endforelse
    </x-welcome.welcome-window>

    <x-welcome.welcome-window section-name="Koncerty, na ktoré ste prihlásený   ">
        @forelse($user->student->user->events as $event)
            <x-welcome.participant-events :$event/>
        @empty
            <p class="text-sm text-slate-500">
                Nie ste prihlásený na žiadne udalosti
            </p>
        @endforelse
    </x-welcome.welcome-window>
